...
#   3. `tauri build --bundles app` → PAVii.app (resources are copied in).
...
# signing key is available — from TAURI_SIGNING_PRIVATE_KEY_PATH, from the env (CI secret
# TAURI_SIGNING_PRIVATE_KEY), or from `.pavii-updater.env` one directory above the repo
# (same convention as the notary env).
...
UPDATER_ENV="${PAVII_UPDATER_ENV:-$PLATFORM/../.pavii-updater.env}"
if [ -z "${TAURI_SIGNING_PRIVATE_KEY_PATH:-}" ] && [ -z "${TAURI_SIGNING_PRIVATE_KEY:-}" ] && [ -f "$UPDATER_ENV" ]; then
