...
.DESCRIPTION
  ...
  3. `tauri build --bundles nsis,msi` -> PAVii NSIS setup .exe + .msi (resources copied in).
...
# signing key env is present. On Windows, prefer setting TAURI_SIGNING_PRIVATE_KEY to the
# private-key file path; multiline key contents can break nested npm/cmd command execution.
# Keyless builds skip the overlay so dev builds keep working; keyless RELEASES strand
# installs without auto-update.
$UpdaterArgs = @()
...
if ($env:TAURI_SIGNING_PRIVATE_KEY) {
    ...
    $Overlay = Join-Path ([IO.Path]::GetTempPath()) "pavii-updater-overlay.json"
...
try {
    ...
    $TauriArgs = @("run", "tauri", "build", "--", "--bundles", $Bundles) + $UpdaterArgs
    & $Npm.Source @TauriArgs
