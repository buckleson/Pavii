...
def _browser_page(
    ...
    return (
        ...
        f"<title>{_html.escape(title)} — PAVii</title><style>"
        ...
        "</style></head><body>"
        '<div class="card"><div class="mark"><i></i>PAVii</div>'
        f"{icon}<h1>{_html.escape(title)}</h1><p>{_html.escape(detail)}</p>{err}</div>"
        '<div class="foot">Served locally by PAVii on your Mac</div>'
        "</body></html>"
    )
...
_CONNECT_FAILED_DETAIL = (
    ...
    "Close this tab and try again from PAVii."
)
...
def create_app(manager: SessionManager) -> FastAPI:
    ...
    async def require_sidecar_token(request: Request, call_next):
        ...
        return JSONResponse(
            {"error": "missing or invalid PAVii sidecar token"},
            status_code=401,
        )
    ...
    async def mcp_oauth_callback(
        ...
        if error:
            return HTMLResponse(
                _browser_page(
                    "Sign-in failed",
                    "The service reported an error. Return to PAVii and try again.",
                    ok=False,
                    error=error,
                ),
                status_code=400,
        ...
        if not code or not mcp_oauth.deliver_callback(code, state or None):
            return HTMLResponse(
                _browser_page(
                    ...
                    "The sign-in may have timed out. Return to PAVii and start it again.",
                    ok=False,
                ),
                status_code=400,
            )
        return HTMLResponse(
            _browser_page(
                "Connected",
                "Sign-in complete. You can close this tab and return to PAVii.",
                ok=True,
            )
        )
    ...
    async def managed_oauth_callback(request: Request) -> Any:
        ...
        if connector == "github" and data.get("installation_id"):
            ...
            return HTMLResponse(
                _browser_page(
                    "GitHub connected",
                    "You can close this tab and return to PAVii.",
                    connector="github",
                )
            )
