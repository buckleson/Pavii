...
test.skip("provider gallery: cards wear their state; Next arms off stored credentials", async ({
  page,
}) => {
...
test("key form: Test verifies, saves, and returns to the gallery with the ✓", async ({
  ...
  await expect(page.getByRole("heading", { name: "Welcome to PAVii" })).toBeVisible();
...
test.skip("key form: revisiting a connected provider shows the in-field saved state; drafts survive switching", async ({
  page,
}) => {
  await openOnboarding(page);
