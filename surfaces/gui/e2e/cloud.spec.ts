...
test.skip(true, 'PAVii Phase 1 removed product account/gallery flow');
...
test("signed out: the account row is the sign-in home; managed connector still connects manually", async ({
  ...
  await expect(row).toContainText("Local mode");

  // The menu leads with the sign-in CTA and always lists Inbox + Connectors.
  await row.click();
  const menu = page.getByTestId("account-menu");
  await expect(menu).toContainText("one-click connections need PAVii connector relay");
  ...
  await expect(modal.getByTestId("managed-connect")).toContainText("Configure PAVii connector access");
...
test("signed in: account row shows the name; one-click appears; sign out from the menu", async ({
  ...
  await expect(page.getByTestId("account-row")).toContainText("Local mode");
});
