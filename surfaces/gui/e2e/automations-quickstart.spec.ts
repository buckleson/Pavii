...
// recipe (role templates, connect rows, direct connector handoff, §25 consent) merged into the page's
...
async function openAutomations(page) {
  ...
  await expect(page.getByText("Recurring tasks PAVii runs on a schedule.")).toBeVisible();
}
...
test("role recipe: connect rows, direct connector handoff, channel by name, consent mints the grant", async ({
  page,
}) => {
  ...
  // Connect HubSpot hands off directly to the connector flow; product account sign-in is gone.
