...
test("kind + persona filters narrow the pending list", async ({ page }) => {
  ...
  // Persona chips render because two personas hold items; filtering to Ops PAVii hides the PAVii item.
  await filters.getByRole("button", { name: "All", exact: true }).click();
  await filters.getByRole("button", { name: "Ops PAVii", exact: true }).click();
  await expect(page.getByText("Approve: run_shell")).not.toBeVisible();
  await expect(page.getByText(question)).toBeVisible();
});
