...
// RECENT-header group/filter popover (Group by Persona↔Chronological, Filter by assistant).
...
test.skip("keyboard shortcut toggles the sidebar collapse", async ({ page }) => {
  await page.goto("/");
  const app = page.locator(".app");
...
test("RECENT header group/filter popover: switch grouping + see assistant filters", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await expect(menu).toContainText("Filter by assistant");

  // Switch to Chronological → the persona accordion collapses into a flat list (the "PAVii"
  ...
  // filter-by-assistant checkboxes are present (none checked by default → all shown).
